        ...
        with io.open(self.input_file, 'r', encoding=self.encoding) as csvfile:
            ...
            for i, row in enumerate(self.cvsreader):
                ...
                try:
                    ...
                    print 'Error while parsing row: {}\n{}\n{}'.format(i+1, row, self.input_file)
                    raise
...
class AliorRorConverter(AbstractRorConverter):
    ...
    def getMemo(self, row):
        memo = ''
        tab = [
            row[self.FIELD_MEMO1].strip(),
            row[self.FIELD_MEMO2].strip(),
            row[self.FIELD_MEMO3].strip(),
            row[self.FIELD_MEMO4].strip()]

        for m in tab:
            if m:
                memo += ' ' + m
        memo += ' | ' + row[self.FIELD_TRANSACTION_TYPE].strip()
        # memo = '%s %s %s %s %s' % (row[self.FIELD_MEMO1].strip(),
        #                            row[self.FIELD_MEMO2].strip(),
        #                            row[self.FIELD_MEMO3].strip(),
        #                            row[self.FIELD_MEMO4].strip(),
        #                            row[self.FIELD_TRANSACTION_TYPE].strip(),)
        # memo = re.sub('Tran.*POL', '', memo).strip()
        # memo = re.sub('Wyp.*POL', '', memo)
        return memo.strip()
...
if __name__ == '__main__':
    ...
    gen_temp = os.path.dirname(__file__) + '/../tests/gen_alior.csv'
    with io.open(gen_temp, mode='w', encoding='utf-8') as of:
        of.write(res)
